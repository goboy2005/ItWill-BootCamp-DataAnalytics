...
select ename,nvl(comm,'no comm')
   from emp;
       ↑ 
an error occurs because 'comm' is an integer while 'no comm' is a text.
so you should chage the integer to text type and then print the result
...
select ename,sal,job, decode(job,'SALESMAN',4500,'ANALYST',2400, 0) as bonus
   from emp;
...
decode(to_date(hiredate,'RRRR'),'1981',5000,'1980',4000, 0) as 보너스 
   from emp;

■ Q-106.이름,월급,보너스를 출력하는데 보너스를 월급이 4000이상이면 500출력 
월급이 2000이상이면 300 출력하고 나머지 월급사원들은 0을 출력하시오  

select ename,sal,case when sal >= 4000 then 500
when sal>= 2000 then 300 else 0 end as 보너스 
    from emp;
...
when deptno = 20 then 300 else 0 end as 보너스
    from emp;

■ Q-108.우리반 테이블에서 이름을 출력하고 그 옆에 보너스를 출력하는데 이름의 철자가 3글자이면 
보너스를 7000을 출력하고, 이름의 철자가 2글자이면 보너스를 5000을 출력
이름의 철자가 4글자이면 보너스를 4000을 출력!        

select ename,case when length(ename)=4 then 4000
when length(ename)=3 then 7000 when length(ename)=2 then 5000
end as 보너스
   from emp12;
...
select ename,to_char(hiredate,'d')
  from emp
...
■ Q-116. 직업,직업별 최대월급을 출력하는데 직업이 SALESMAN 만 출력하시오 ! 
