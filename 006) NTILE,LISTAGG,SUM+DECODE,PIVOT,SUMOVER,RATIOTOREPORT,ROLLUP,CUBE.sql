...
설명:  within ~이내에 
월급이 높은순서데로 정렬한 그룹 안에서 2975가 몇위의 순위인가.
...
등급이 3등급으로 나눠지게 하시오 !
(등급은 월급이 높은 순서가 기준 등급이다)

select  job, ename, sal, ntile(3) over   
(partition by job order  by  sal  desc ) 등급
    from  emp;
...
select  ename, hiredate,lag(hiredate,1) over ( order by hiredate asc) 전행,
